...
```{r eval=TRUE, message=FALSE, warning=FALSE, out.width='300px'}
...
```{r eval=FALSE, message=FALSE, warning=FALSE, out.width='300px'}
#Control parameters:
...
```{r eval=TRUE, message=FALSE, warning=FALSE, include=FALSE}
model_1<-readRDS("model_1")
model_2<-readRDS("model_2")
model_3<-readRDS("model_3")
model_4<-readRDS("model_4")
```
...
The model RF again seems to give high accuracy in the validation set (good generalization). The version without preprocessing does slightly better for the Classes A, B, and E, while the version with preprocessing predicts slightly better the other two classes (C and D) and has better overall Accuracy. Comparing the Accuracies for the validation test though, we notice that the xgbTree model slightly outperforms the RF one. Higher Accuracy means lower out of sample Error rate.
